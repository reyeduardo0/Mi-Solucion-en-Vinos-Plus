...
const AddPackModelModal: React.FC<{ supplies: Supply[]; products: { name: string }[]; onSave: (model: Omit<PackModel, 'id' | 'created_at'>) => void; onClose: () => void; }> = ({ supplies, products, onSave, onClose }) => {
...
const PackModels: React.FC = () => {
    const { packModels, supplies, addPackModel, products } = useData();
    const [showModal, setShowModal] = useState(false);

    return (
        <div className="p-4 sm:p-6 lg:p-8">
            {showModal && <AddPackModelModal products={products} supplies={supplies} onSave={addPackModel} onClose={() => setShowModal(false)} />}
                    ...
                    <Card key={model.id} className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
                        ...
                        <div className="space-y-3 text-sm bg-gray-50 p-3 rounded-md border">
                            <div><h4 className="flex items-center text-sm font-semibold text-gray-700 mb-1"><BoxIcon/> Productos Requeridos</h4><ul className="list-disc list-inside pl-4 text-gray-600">{model.productRequirements?.map((p, i) => <li key={i}>{p.productName} (x{p.quantity})</li>)}</ul></div>
                             <div className="pt-2 border-t"><h4 className="flex items-center text-sm font-semibold text-gray-700 mb-1"><SupplyIcon/> Consumibles Requeridos</h4><ul className="list-disc list-inside pl-4 text-gray-600">{model.supplyRequirements?.map((s, i) => <li key={i}>{s.name} (x{s.quantity})</li>)}</ul></div>
                        </div>
                    </Card>
                )) : (
