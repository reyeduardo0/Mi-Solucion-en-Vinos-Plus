...
import { PackModelIcon } from '../constants'; // Import the icon for the empty state
...
const AddPackModelModal: React.FC<{
    supplies: Supply[];
    onSave: (model: Omit<PackModel, 'id'>) => void;
    onClose: () => void;
}> = ({ supplies, onSave, onClose }) => {
        ...
        <Modal title="Crear Nuevo Modelo de Pack" onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-8">
                <fieldset className="space-y-4">
                    <legend className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">Datos Generales</legend>
                    <div><label className="block text-sm font-medium">Nombre del Modelo</label><input type="text" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full border-gray-300 rounded-md shadow-sm p-2" /></div>
                    <div><label className="block text-sm font-medium">Descripción</label><textarea value={description} onChange={e => setDescription(e.target.value)} required rows={2} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm p-2" /></div>
                </fieldset>

                <fieldset>
                    <legend className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">Requisitos de Productos</legend>
                    <div className="space-y-3">
                        {productReqs.map((p, i) => (
                            <div key={i} className="grid grid-cols-12 gap-2 items-center"><div className="col-span-8"><input type="text" placeholder="Nombre del producto" value={p.productName} onChange={e => handleUpdateProductReq(i, 'productName', e.target.value)} className="w-full p-2 border rounded-md" /></div><div className="col-span-3"><input type="number" value={p.quantity} onChange={e => handleUpdateProductReq(i, 'quantity', e.target.value)} min="1" className="w-full p-2 border rounded-md" /></div><div className="col-span-1"><button type="button" onClick={() => handleRemoveProductReq(i)} className="p-2 text-red-500 hover:bg-red-100 rounded-full"><TrashIcon /></button></div></div>
                        ))}
                    </div>
                    <Button type="button" variant="secondary" onClick={handleAddProductReq} className="mt-3 text-sm py-1.5">Añadir Producto</Button>
                </fieldset>
                
                <fieldset>
                    <legend className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">Requisitos de Insumos</legend>
                    <div className="space-y-3">
                         {supplyReqs.map((s, i) => (
                            <div key={i} className="grid grid-cols-12 gap-2 items-center"><div className="col-span-8"><select value={s.supplyId} onChange={e => handleUpdateSupplyReq(i, 'supplyId', e.target.value)} className="w-full p-2 border rounded-md"><option value="">Seleccionar Insumo...</option>{supplies.filter(sup => sup.type === 'Contable').map(sup => <option key={sup.id} value={sup.id}>{sup.name}</option>)}</select></div><div className="col-span-3"><input type="number" value={s.quantity} onChange={e => handleUpdateSupplyReq(i, 'quantity', e.target.value)} min="1" className="w-full p-2 border rounded-md" /></div><div className="col-span-1"><button type="button" onClick={() => handleRemoveSupplyReq(i)} className="p-2 text-red-500 hover:bg-red-100 rounded-full"><TrashIcon /></button></div></div>
                        ))}
                    </div>
                    <Button type="button" variant="secondary" onClick={handleAddSupplyReq} className="mt-3 text-sm py-1.5">Añadir Insumo</Button>
                </fieldset>
...
interface PackModelsProps {
    models: PackModel[];
    supplies: Supply[];
    onAddModel: (model: Omit<PackModel, 'id'>) => void;
}

const PackModels: React.FC<PackModelsProps> = ({ models, supplies, onAddModel }) => {
    const [showModal, setShowModal] = useState(false);

    return (
        <div className="p-4 sm:p-6 lg:p-8">
            {showModal && <AddPackModelModal supplies={supplies} onSave={onAddModel} onClose={() => setShowModal(false)} />}
                ...
                {models.length > 0 ? models.map(model => (
                    <Card key={model.id} className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
                        <div className="flex justify-between items-start mb-1">
                            <h3 className="text-lg font-bold text-gray-900 pr-4">{model.name}</h3>
                             <div className="flex space-x-1 flex-shrink-0">
                                 <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-gray-100 rounded-full transition-colors"><PencilIcon /></button>
                                 <button className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-gray-100 rounded-full transition-colors"><TrashIcon /></button>
                             </div>
                        </div>
                        ...
                        <div className="space-y-3 text-sm bg-gray-50 p-3 rounded-md border">
                            <div>
                                <h4 className="flex items-center text-sm font-semibold text-gray-700 mb-1"><BoxIcon/> Productos Requeridos</h4>
                                <ul className="list-disc list-inside pl-4 text-gray-600">
                                    {model.productRequirements.map((p, i) => <li key={i}>{p.productName} (x{p.quantity})</li>)}
                                </ul>
                            </div>
                             <div className="pt-2 border-t">
                                <h4 className="flex items-center text-sm font-semibold text-gray-700 mb-1"><SupplyIcon/> Insumos Requeridos</h4>
                                <ul className="list-disc list-inside pl-4 text-gray-600">
                                    {model.supplyRequirements.map((s, i) => <li key={i}>{s.name} (x{s.quantity})</li>)}
                                </ul>
                            </div>
                        </div>
                    </Card>
                    ...
                    <div className="md:col-span-2 lg:col-span-3">
                        <Card className="text-center py-12 flex flex-col items-center border-2 border-dashed">
                             <div className="bg-gray-100 rounded-full p-4">
                                <PackModelIcon />
                            </div>
                            <h3 className="text-xl font-semibold text-gray-700 mt-4">No hay modelos de pack definidos</h3>
                            <p className="text-gray-500 mt-2">Crea tu primer modelo para empezar a estandarizar tus packs.</p>
