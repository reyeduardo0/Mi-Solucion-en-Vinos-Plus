...
interface AddPackModelModalProps {
    supplies: Supply[];
    products: { name: string }[];
    onSave: (model: Omit<PackModel, 'id' | 'created_at'> | PackModel) => void;
    onClose: () => void;
    initialModel?: PackModel | null;
}

const AddPackModelModal: React.FC<AddPackModelModalProps> = ({ supplies, products, onSave, onClose, initialModel }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const modelData = { name, description, productRequirements: productReqs, supplyRequirements: supplyReqs };
        if (initialModel) {
            onSave({ ...initialModel, ...modelData });
        } else {
            onSave(modelData);
        }
        onClose();
    ...
    return (
        <Modal title={initialModel ? "Editar Modelo de Pack" : "Crear Nuevo Modelo de Pack"} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <fieldset><legend className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">Requisitos de Productos</legend><div className="space-y-3">{productReqs.map((p, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center"><div className="col-span-8"><input type="text" placeholder="Nombre del producto" value={p.productName} onChange={e => handleUpdateProductReq(i, 'productName', e.target.value)} className="w-full p-2 border rounded-md" list="product-list" /><datalist id="product-list">{products.map(p => <option key={p.name} value={p.name} />)}</datalist></div><div className="col-span-3"><input type="number" value={p.quantity} onChange={e => handleUpdateProductReq(i, 'quantity', Number(e.target.value))} min="1" className="w-full p-2 border rounded-md" /></div><div className="col-span-1"><button type="button" onClick={() => handleRemoveProductReq(i)} className="p-2 text-red-500 hover:bg-red-100 rounded-full"><TrashIcon /></button></div></div>))}<Button type="button" variant="secondary" onClick={handleAddProductReq} className="mt-3 text-sm py-1.5">Añadir Producto</Button></div></fieldset>
                <fieldset><legend className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">Requisitos de Consumibles</legend><div className="space-y-3">{supplyReqs.map((s, i) => (<div key={i} className="grid grid-cols-12 gap-2 items-center"><div className="col-span-8"><select value={s.supplyId} onChange={e => handleUpdateSupplyReq(i, 'supplyId', e.target.value)} className="w-full p-2 border rounded-md"><option value="">Seleccionar Consumible...</option>{supplies.filter(sup => sup.type === 'Contable').map(sup => <option key={sup.id} value={sup.id}>{sup.code ? `[${sup.code}] ` : ''}{sup.name}</option>)}</select></div><div className="col-span-3"><input type="number" value={s.quantity} onChange={e => handleUpdateSupplyReq(i, 'quantity', Number(e.target.value))} min="1" className="w-full p-2 border rounded-md" /></div><div className="col-span-1"><button type="button" onClick={() => handleRemoveSupplyReq(i)} className="p-2 text-red-500 hover:bg-red-100 rounded-full"><TrashIcon /></button></div></div>))}<Button type="button" variant="secondary" onClick={handleAddSupplyReq} className="mt-3 text-sm py-1.5">Añadir Consumible</Button></div></fieldset>
...
const PackModels: React.FC = () => {
    const { packModels, supplies, addPackModel, updatePackModel, deletePackModel, products } = useData();
    const [showModal, setShowModal] = useState(false);
    const [modelToEdit, setModelToEdit] = useState<PackModel | null>(null);
    const [modelToDelete, setModelToDelete] = useState<PackModel | null>(null);

    const handleSave = async (model: Omit<PackModel, 'id' | 'created_at'> | PackModel) => {
        try {
            if ('id' in model) {
                await updatePackModel(model);
            } else {
                await addPackModel(model);
            }
            setShowModal(false);
            setModelToEdit(null);
        } catch (error) {
    ...
    const handleEdit = (model: PackModel) => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await deletePackModel(modelToDelete.id, modelToDelete.name);
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            {modelToDelete && (
                <ConfirmationModal 
                    title="Eliminar Modelo de Pack" 
                    message={`¿Estás seguro de que quieres eliminar el modelo "${modelToDelete.name}"? Esta acción no se puede deshacer.`}
                    ...
                    <Card key={model.id} className="flex flex-col h-full hover:shadow-lg transition-shadow duration-300">
                        <div className="flex justify-between items-start mb-1">
                            ...
                            <div className="flex space-x-1 flex-shrink-0">
                                <button onClick={() => handleEdit(model)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-gray-100 rounded-full transition-colors"><PencilIcon /></button>
                                <button onClick={() => setModelToDelete(model)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-gray-100 rounded-full transition-colors"><TrashIcon /></button>
                            </div>
                        </div>
