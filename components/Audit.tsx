...
const ActionIcon: React.FC<{ action: string }> = ({ action }) => {
    ...
    if (lowerCaseAction.includes('creó') || lowerCaseAction.includes('registró') || lowerCaseAction.includes('añadió')) {
        icon = <CreateIcon />;
        color = 'bg-green-500';
    } else if (lowerCaseAction.includes('actualizó') || lowerCaseAction.includes('editó') || lowerCaseAction.includes('resolvió')) {
        icon = <UpdateIcon />;
        color = 'bg-blue-500';
    } else if (lowerCaseAction.includes('eliminó')) {
        icon = <DeleteIcon />;
        color = 'bg-red-500';
    } else if (lowerCaseAction.includes('inició sesión')) {
        icon = <LoginIcon />;
        color = 'bg-purple-500';
    }

    return (
        <div className={`z-10 flex items-center justify-center w-8 h-8 ${color} rounded-full ring-4 ring-white`}>
...
const Audit: React.FC<AuditProps> = ({ logs, users }) => {
            ...
            <Card title="Filtros de Búsqueda" className="mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="userId" value={filters.userId} onChange={handleFilterChange} className="w-full p-2 border rounded-md">
                            <option value="">Todos los usuarios</option>
                            {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Acción Contiene</label>
                        <input type="text" name="actionQuery" value={filters.actionQuery} onChange={handleFilterChange} placeholder="Ej: eliminó, albarán..." className="w-full p-2 border rounded-md" />
                    </div>
                    <div>
                ...
                <div className="flow-root">
                    {filteredLogs.length > 0 ? (
                        ...
                        <div className="text-center py-12 flex flex-col items-center">
                            <EmptyIcon />
                            <h3 className="text-xl font-semibold text-gray-700 mt-4">No se encontraron registros</h3>
                            <p className="text-gray-500 mt-2">No hay registros de auditoría que coincidan con los filtros aplicados.</p>
                        </div>
                    )}
                </div>
            </Card>
