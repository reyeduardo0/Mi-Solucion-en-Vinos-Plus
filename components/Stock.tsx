...
const AddSupplyModal: React.FC<{ 
    supply?: Supply | null;
    onSave: (supplyData: Omit<Supply, 'id' | 'created_at' | 'quantity'> | Supply, initialData?: { quantity: number, lot: string }) => void; 
    onClose: () => void; 
}> = ({ supply, onSave, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!supply && initialQtyNum > 0) {
            initialData = { quantity: initialQtyNum, lot: initialLot };
        }

        if (supply) {
            onSave({ ...supply, ...supplyData });
        } else {
...
const Inventory: React.FC = () => {
    const { supplies, addNewSupply, addSupplyStock, inventoryStock, updateSupply, deleteSupply } = useData();
    ...
    const handleSaveSupply = (
        ...
        const saveAction = 'id' in data 
            ? updateSupply(data) 
            : addNewSupply(data, initialData);
        saveAction.catch(err => alert(err.message));
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            {(showAddSupplyModal || supplyToEdit) && <AddSupplyModal supply={supplyToEdit} onSave={handleSaveSupply} onClose={() => { setShowAddSupplyModal(false); setSupplyToEdit(null); }} />}
