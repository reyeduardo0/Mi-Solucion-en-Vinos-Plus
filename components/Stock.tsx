...
const Inventory: React.FC = () => {
    const { albaranes, supplies, packs, mermas, addNewSupply, addSupplyStock } = useData();
    ...
    const inventoryData = useMemo(() => {
        ...
        packs.filter(p => p.status === 'Ensamblado').forEach(p => {
            p.contents.forEach(c => {
                ...
                if (stockMap.has(key)) stockMap.get(key)!.inPacks += c.quantity;
            });
            p.suppliesUsed?.forEach(s => {
                const supplyDef = supplies.find(sup => sup.id === s.supplyId);
                if(supplyDef) {
                    const key = `supply-${supplyDef.name}`;
                    if (stockMap.has(key)) stockMap.get(key)!.inPacks += s.quantity;
                }
            });
        });
        ...
        mermas.forEach(m => {
            ...
            if (stockMap.has(key)) stockMap.get(key)!.inMerma += m.quantity;
        });
